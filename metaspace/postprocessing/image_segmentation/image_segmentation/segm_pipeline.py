...
def run_segmentation(  # pylint: disable=too-many-arguments,too-many-locals
    ...
    if input_s3_key is not None:
        ...
        logger.info(
            f"Dataset {dataset_id}: loading input via Python client (database_ids={database_ids})"
        )
    ...
    logger.info(
        f"[SEGMENTATION_PERF] Algorithm dispatch ({algorithm}) completed in {dispatch_time:.3f}s "
        f"for dataset {dataset_id}"
    )
