...
function pieScatterPlot(svg: any, data: any, config: PieChartConfig, xData: EdgeHistogram, yData: EdgeHistogram) {
  ...
  const xScale = d3
    .scaleBand()
    .domain(xData.map((x) => x.key))
    .rangeRound([0, width])
  const yScale = d3
    .scaleBand()
    .domain(yData.map((x) => x.key))
    .rangeRound([height, 0])
  ...
  if (config.showSideHistograms) {
    if (config.showSideHistograms.x) {
      const xL = d3
        .scaleLinear()
        .domain([0, d3.max(yData.map((d) => d.count))!])
        .range([0, -margin.left / 3 + 1])
      const yL = d3
        .scaleBand()
    ...
    if (config.showSideHistograms.y) {
      ...
      svg
        .append('g')
        .selectAll('rect.bhist')
        .data(xData)
        .enter()
        .append('rect')
        .attr('class', 'bhist')
        .attr('y', yL(0))
        .attr('x', (d: any): number => xL(d.key) || 0)
        .attr('width', (): number => xL.bandwidth() - 1)
  ...
  addMainTitle(svg, geometry, mainTitle).attr('font-size', '16px')
...
export { addAxes, addLegend, addMainTitle, configureSvg, pieScatterPlot, setTickSize }
