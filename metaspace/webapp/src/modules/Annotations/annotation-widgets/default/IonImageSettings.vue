  ...
  <span id="ion-image-settings">
    <el-form label-position="top">
      <el-form-item label="Intensity scaling" data-feature-anchor="ion-image-color-scale">
        <el-select :model-value="scaleType" style="width: 300px" :teleported="false" @change="onScaleTypeChange">
          <el-option value="linear" label="Linear" />
          <el-option value="linear-full" label="Linear (without hot-spot removal)" />
      ...
      <el-form-item v-if="showIntensityTemplate" data-feature-anchor="global-intensity-lock">
        <template v-slot:label>
          <el-popover trigger="hover" placement="right">
            ...
            <div class="max-w-xs">Apply intensities range of one dataset to all other datasets.</div>
          </el-popover>
        </template>
        <el-select
        ...
        >
          <el-option v-for="item in lockTemplateOptions" :key="item.id" :label="item.name" :value="item.id">
          </el-option>
        </el-select>
      </el-form-item>
        ...
        >
          <el-option-group>
            <el-option
              ...
              :label="scale"
            >
              <color-bar class="h-5 w-full" :map="scale" horizontal />
            </el-option>
          </el-option-group>
      ...
      <el-form-item label="Scale bar color">
        <el-select :model-value="pickedColor" style="width: 150px" :teleported="false" @change="onScaleBarColorChange">
          <el-option v-for="color in paletteColors" :key="color.code" :value="color.code" :label="color.colorName">
            <div :style="{ position: 'relative', background: color.bgColor, height: '50px' }">
              <scale-bar :x-scale="8" :y-scale="8" :scale-bar-color="color.code" />
            </div>
          </el-option>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const pickedColor = ref<string>(
      props.defaultScaleBarColor ? props.defaultScaleBarColor : paletteColors.value[0].code
    )
