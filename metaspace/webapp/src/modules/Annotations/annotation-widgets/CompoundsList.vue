<template>
  <div class="compound-list">
    <div v-for="(compound, idx) in compounds" :key="idx" class="compound">
      <el-popover placement="left" trigger="click">
        ...
        <div class="compound-popover">
          <figure class="flex flex-col items-center justify-center">
            ...
            <NoImageURL v-if="failedImages.includes(compound.imageURL)" />
            <img v-else :src="compound.imageURL" class="compound-image" />
          </figure>
        </div>
      </el-popover>
      <br />
      ...
      <span v-else>
        <a :href="compound.information[0].url" target="_blank" :title="compound.name">
          {{ compound.name.slice(0, 32) + '...' }}
        </a>
      </span>
    </div>
    <div v-if="compounds.length === 0" class="empty-message">This database does not contain molecule information.</div>
  </div>
</template>
...
const NoImageURL = defineAsyncComponent(() => import('../../../assets/no-image.svg'))
