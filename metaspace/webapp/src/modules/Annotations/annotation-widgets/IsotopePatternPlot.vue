<template>
  <div class="peak-chart w-full">
    <div ref="peakChart" style="height: 300px" />
    <div v-if="legendItems" class="plot-legend">
      <div v-for="(item, idx) in legendItems" :key="idx" class="legend-item">
        ...
        <span class="legend-item-name" v-html="item.labelHtml" />
      </div>
    </div>
  </div>
</template>
...
class SampleGraph {
  constructor(svg, xScale, yScale, points, cssClass, ppm) {
    ...
    this.circles = element.append('g').selectAll('circle').data(points).enter().append('circle')

    this.lines = element.append('g').selectAll('line').data(points).enter().append('line')

    this.ppmRectangles = element.append('g').selectAll('rect').data(points).enter().append('rect')
...
function plotChart(data, element, relativeIntensityScale, toggleRelativeIntensityScale) {
  ...
  const yAxis = d3
    .axisLeft(yScale)
    .ticks(5, relativeIntensityScale ? null : 'e')
    .tickPadding(5)
  ...
  function brushHandler(event) {
    ...
    const s = event.selection // Use the event argument instead of d3.event
    ...
    if (!s) {
      ...
      function calcMaxIntensity(pts) {
        if (pts) {
          const intensities = pts.filter((d) => d[0] >= mzRange[0] && d[0] <= mzRange[1]).map((d) => d[1])
          if (intensities.length > 0) {
            return d3.max(intensities)
          } else {
            // no points in range - get the intensity of the closest point to the selected range
            return pts
              .map((d) => [Math.abs(d[0] - mzRange[0]), d[1]])
              .reduce((a, r) => (a[0] < r[0] ? a : r), [Infinity, 100])[1]
          }
        }
        return 0
      }
      const maxInt = d3.max([...pointss.map(calcMaxIntensity), ...theorPointss.map(calcMaxIntensity)])
  ...
  const svg = container.append('g').attr('transform', `translate(${margin.left}, ${margin.top})`)
  ...
  const theorGraphs = theorPointss.map(
    (theorPoints, i) => new TheorGraph(svg, xScale, yScale, theorPoints, theorClasses[i])
  )
...
const makeLegendTheorLine = () => {
  const curve = [
    0, 0, 0, 0, 0, 1, 2, 4, 9, 16, 27, 42, 59, 77, 91, 100, 100, 92, 77, 60, 42, 28, 16, 9, 5, 2, 1, 0, 0, 0, 0, 0,
  ]
  const drawPath = d3
    .line()
    .x((d) => d * 2)
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const relativeIntensityScale = ref(true)

    const legendTheorLine = makeLegendTheorLine()

    const redraw = throttle(() => {
      plotChart(props.data, peakChart.value, relativeIntensityScale.value, toggleRelativeIntensityScale)
    }, 200)
    ...
    function toggleRelativeIntensityScale() {
      relativeIntensityScale.value = !relativeIntensityScale.value
      redraw()
    }
...
.peak-chart ::v-deep(.intensity-axis-label) {
  text-anchor: middle;
...
.peak-chart ::v-deep(.sample-graph) {
  circle {
    stroke: none;
  }
