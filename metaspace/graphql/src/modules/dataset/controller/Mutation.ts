...
import { projectToLegacyMetadata, MetadataV2Envelope } from '../projection'
...
// Same plain-`require` reasoning as Ajv04 above. The vendored schema declares
// "$schema": "https://json-schema.org/draft/2019-09/schema" (LinkML's jsonschemagen default),
// which plain `ajv@8` doesn't validate against out of the box - `ajv/dist/2019` is the build with
// the 2019-09 meta-schema/vocabulary wired in.
// eslint-disable-next-line @typescript-eslint/no-var-requires
const Ajv2019 = require('ajv/dist/2019')
// Vendored from metaspace_metadata (schema repo) per metaspace/metadata/linkml/SCHEMA_VERSION -
// not generated by any step in this package, see that file for provenance/pinned commit.
// eslint-disable-next-line @typescript-eslint/no-var-requires
const metadataV2Schema = require('../../../../../metadata/linkml/generated/metaspace_submission.schema.json')
...
//
// `metadataV2`, when supplied, is validated two ways: (1) `metadataV2.document` against the
// vendored generated JSON Schema (ajv@8/2019-09, see metaspace/metadata/linkml/SCHEMA_VERSION for
// provenance) - the authoritative check that the new-schema document itself is well-formed; and
// (2) belt-and-braces, down-project it back to the legacy shape (same function used for the actual
// write path, see createDataset/updateDataset) and run that through the existing ims.json/Ajv04
// validator below, catching a document that's schema-valid but wouldn't down-project to something
// legacy-valid. `metadataV2.unmodeled` has no separate schema of its own - its fields
// (Detector_Resolving_Power, Pixel_Size, Submitted_By, Supplementary) are already covered by (2),
// since the down-projection carries them into the legacy shape under their legacy field names.
export function validateMetadata(metadata: MetadataNode, metadataV2?: MetadataV2Envelope) {
  ...
  if (metadataV2) {
    ...
    const v2Validator = ajv2019.compile(metadataV2Schema)
    /* eslint-disable-next-line @typescript-eslint/no-floating-promises */
    v2Validator(metadataV2.document)
    validationErrors.push(...(v2Validator.errors || []).map(translateAjvError))

    const projected = projectToLegacyMetadata(metadataV2)
    const projectedCleanValue = trimEmptyFields(mdSchema, projected)
    /* eslint-disable-next-line @typescript-eslint/no-floating-promises */
    validator(projectedCleanValue)
    validationErrors.push(...(validator.errors || []).map(translateAjvError))
  }
...
const createDataset = async(args: CreateDatasetArgs, ctx: Context) => {
  ...
  const metadataV2: MetadataV2Envelope | undefined = input.metadataV2Json
    ? JSON.parse(input.metadataV2Json)
    : undefined
  if (!skipValidation || !ctx.isAdmin) {
    validateMetadata(metadata, metadataV2)
  }
  ...
  await smApiDatasetRequest(url, {
    doc: {
      ...
      metadata,
      metadataV2,
      size_hash: input.sizeHashJson ? JSON.parse(input.sizeHashJson) : undefined,
    },
    priority: priority,
...
const MutationResolvers: FieldResolversFor<Mutation, void> = {
  ...
  reprocessDataset: async(source, {
    ...
    return await createDataset({
      ...
      input: {
        ...
        metadataV2Json: engineDataset.metadataV2 ? JSON.stringify(engineDataset.metadataV2) : undefined,
  ...
  updateDataset: async(source, args, ctx: Context) => {
    ...
    let metadata
    let metadataV2: MetadataV2Envelope | undefined
    if (update.metadataV2Json) {
      metadataV2 = JSON.parse(update.metadataV2Json)
    }
    ...
    if (reprocess) {
      ...
      if (reprocessingNeeded) {
        ...
        await smApiUpdateDataset(datasetId, {
          ...
          ..._.omit(update, ['metadataJson', 'metadataV2Json']) as any,
          ...(metadata ? { metadata } : {}),
          ...(metadataV2 ? { metadataV2 } : {}),
        }, {
          priority,
