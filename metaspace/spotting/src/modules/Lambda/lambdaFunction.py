...
def load_data(is_matrix_data=False, load_pathway=False, load_class=False, filters={}):
...
def summarise_data(df, x_axis, y_axis, intensity_col_name, prediction_col_name):
    ...
    """
    intensity_aggregation_func = lambda x: 0 if True not in (x == 2).value_counts().index else \
    (x == 2).value_counts().loc[True]
    ...
    data = df.pivot_table(index=[x_axis],
                          columns=y_axis,
                          values=[intensity_col_name, prediction_col_name, 'class_size', 'log', 'dataset_ids',
                                  'formulas', 'matrixes'],
                          aggfunc={
                              'class_size': sum,
