<template>
  <el-popover :key="copied" trigger="hover" placement="top" popper-class="min-w-0" @after-leave="copied = false">
    <template #reference>
      <button
        ...
        @click="handleCopy"
      >
        <copy-id-icon v-if="isId" class="h-full w-full text-gray-400 hover:text-gray-500 sm-copy-icon" />
        <el-icon v-else class="h-full w-full text-gray-400 hover:text-gray-500 sm-copy-icon">
          <CopyDocument />
        </el-icon>
      </button>
    </template>
    <p class="leading-none m-0">
      ...
      <slot v-else> Copy to clipboard </slot>
    </p>
  </el-popover>
</template>
