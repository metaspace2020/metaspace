...
export default defineComponent({
  ...
  setup() {
    ...
    const query = gql`query GetUploadTimes($filter: DatasetFilter, $query: String) {
      allDatasets(filter: $filter, simpleQuery: $query, limit: 50000) {
        uploadDateTime
      }
    }`;

    const { result: uploadDatesResult, loading, refetch } = useQuery(query,
      () => ({
    ...
    watch(uploadDates, (newDates) => {
      const dates = newDates.filter(x => x !== null)
