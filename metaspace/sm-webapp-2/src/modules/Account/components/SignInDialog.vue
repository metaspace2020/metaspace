<template>
  <el-dialog class="max-w-2xl" title="Sign in" v-model="model.dialogVisible" :lock-scroll="false" @close="onClose">
    <div class="container">
      <div class="left-side">
        <el-form ref="form" :model="model" :rules="rules">
          <el-form-item prop="email" :validate-status="isInvalid ? 'error' : undefined">
            <el-input v-model="model.email" placeholder="Email address" />
          </el-form-item>
          <el-form-item prop="password" :validate-status="isInvalid ? 'error' : undefined">
            <el-input v-model="model.password" type="password" placeholder="Password" @keypress.enter="onSubmit" />
          </el-form-item>
          <vue3-slide-up-down v-model="isInvalid">
            <p style="margin-top: 0; color: red">Incorrect username or password</p>
          </vue3-slide-up-down>
    ...
    <p>
      <inter-dialog-link dialog="forgotPassword"> Forgot password? </inter-dialog-link>
    </p>
    <p>New user? <inter-dialog-link dialog="createAccount"> Create an account </inter-dialog-link></p>
    <p style="margin-bottom: 0">
      Submitted data previously but never set a password?
      <a href="#" @click.prevent="togglePreMigrationUserInfo">Click here</a>
    </p>
    <vue3-slide-up-down v-model="showPreMigrationUserInfo">
      <p style="margin-bottom: 0">
        We now require that all METASPACE users have an account to upload and manage datasets. If you have submitted
