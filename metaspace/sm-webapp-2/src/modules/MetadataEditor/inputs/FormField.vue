<template>
  ...
  >
    <template v-slot:label>
      <span
        class="field-label flex items-center"
      >
          <span>{{ name }}</span><span
          v-if="required"
          style="color: red"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const optionsAreStrings = computed(() => props.options && props.options.length > 0 && typeof props.options[0] === 'string');
    const createOptionsAreStrings = computed(() => createItemPreviewOptions.value.length > 0 && typeof createItemPreviewOptions.value[0] === 'string');
    ...
    const fetchSuggestionsAndTestWidth = (queryString: string, callback: any) => {
      ...
      props.fetchSuggestions(queryString, results => {
        wideAutocomplete.value = Array.isArray(results)
