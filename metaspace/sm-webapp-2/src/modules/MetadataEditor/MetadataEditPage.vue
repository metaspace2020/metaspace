...
export default defineComponent({
  ...
  setup() {
    ...
    const confirmSkipValidation = async() => {
      try {
        await ElMessageBox.confirm('There were validation errors. Save anyway?',
          'Validation errors',
          {
            type: 'warning',
            confirmButtonText: 'Continue',
    ...
    const confirmForce = async() => {
      try {
        await ElMessageBox.confirm('This dataset is queued or processing. Save anyway?',
          'Dataset busy',
          {
            type: 'warning',
            confirmButtonText: 'Continue',
      ...
      try {
        ...
        if (get(err, 'graphQLErrors[0].isHandled')) {
          ...
          if (systemHealth.value && (!systemHealth.value.canProcessDatasets || !systemHealth.value.canMutate)) {
            ElMessageBox.alert(`Changes to the analysis options require that this dataset be reprocessed; however,
