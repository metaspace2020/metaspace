  ...
  setup(props) {
    const isotopeImages = computed(() => {
      const images = sortBy(props.annotation.isotopeImages, img => img.mz)
        .filter(img => img.mz > 0)
        .map((img, i) => {
          const overlapsPeaks = (props.isobarPeakNs || [])
            .filter(([a]) => a === i + 1)
            .map(([, b]) => b);
          const isobarAlert = overlapsPeaks.length > 0
            ? `This peak's detection window overlaps with the other annotation's ${formatHumanReadableList(overlapsPeaks.map(formatNth))}${overlapsPeaks.length > 1 ? ' peaks.' : ' peak.'}`
            : null;
