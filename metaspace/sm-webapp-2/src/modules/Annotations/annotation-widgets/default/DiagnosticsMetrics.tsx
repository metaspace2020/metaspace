...
export default defineComponent({
  ...
  setup(props) {
    const metrics = computed(() => {
      ...
      if (metrics != null) {
        const sorted = sortBy(Object.entries(metrics).filter(([k]) => order.includes(k)), ([k]) => order.indexOf(k));
        metrics = Object.fromEntries(sorted);
      }
      return metrics;
    });

    const scoringModel = computed(() => props.annotation != null ?
      safeJsonParse(props.annotation.dataset.configJson)?.fdr.scoring_model : null);
