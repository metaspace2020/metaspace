...
const relatedAnnotation = {
  ...
  fdrLevel: 0.2,
  possibleCompounds: [testCompound('Maltoxazine', 'HMDB0030372'), testCompound('Salsolinol', 'HMDB0042012')],
}

vi.mock('@vue/apollo-composable', () => ({
  useQuery: vi.fn(() => ({
    result: ref({ allAnnotations: [relatedAnnotation, referenceAnnotation] }),
    loading: ref(false),
    error: ref(null),
...
describe('RelatedMolecules', () => {
  ...
  it('should hide the isomer when the feature flag is off', async () => {
    ...
    expect(wrapper.findAll('.compound').map((el) => el.text())).toEqual([expect.stringContaining('Tryptophanol')])
  })
})
