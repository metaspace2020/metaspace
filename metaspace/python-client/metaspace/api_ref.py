...
def get_study_info(ds_ids, database='HMDB-v4', fdr=0.1, config=None):
    ...
    for ii, ds_id in enumerate(ds_ids):
        ...
        files.update(
            {'optical': {'image': ds._baseurl + opt_im['url'], 'transform': opt_im['transform']}}
        )
    ...
    annotations = (
        pd.DataFrame.from_records([[a for a in an] for an in annotations])
        .pivot_table(index=[0], columns=[4, 3, 2, 1], values=5)
        .fillna(0)
        .T
    )
