...
class GraphQLClient(object):
    def __init__(self, url, jwt=""):
        self.url = url
        self.jwt=jwt
        # Great big hacking hacks
        # TODO: fix this once the LC fork is merged
        if "metaspace2020.eu" in url:
            self.dstype = 'imaging'
            self.DATASET_FIELDS = self.DATASET_FIELDS.replace("acquisitionGeometry","")
        else:
            self.dstype = 'lc'
...
class SMDataset(object):
    ...
    @property
    def acquisitionGrid(self):
        return np.asarray(json.loads(self._info['acquisitionGeometry'])['acquisition_grid']['coord_list'])

    def __repr__(self):
        return "SMDataset({} | ID: {})".format(self.name, self.id)

    def annotations(self, fdr=0.1, database=None, return_vals = ['sumFormula', 'adduct', 'mz']):
    ...
    def isotope_images(self, sf, adduct):
        ...
        def fetchTrace(url):
            ...
            bytes = BytesIO(requests.get(url).content)
            return np.fromstring(bytes.read(), dtype=np.uint8).astype(float)

        images = []
        image_metadata = None
        if records:
            image_metadata = records[0]['isotopeImages']
            #print(self._gqclient.dstype)
            if self._gqclient.dstype == 'imaging':
                images = [fetchImage(r.get('url')) for r in image_metadata]
            elif self._gqclient.dstype == 'lc':
                images = [fetchTrace(r.get('url')) for r in image_metadata]
