...
class NewPngHandler(tornado.web.RequestHandler):
    ...
    Available at url /demo-png. Caches the res_dict until a request arrives that requires computing a different
    res_dict.

    The min and max intensity is also cached for the colorbar. It is required that the client download the total ion
     image before requesting the colorbar, since the min and max values are computed during the image generation."""
    cache = {}
    minmax_cache = {}
    ...
    def get(self, dataset_id, job_id, sf_id, sf, adduct=None, peak_id=None):
        ...
        if self.request.uri.split('/')[1] == "mzimage_meta":
            ...
            return
        colormap = ((0x35, 0x2A, 0x87), (0x02, 0x68, 0xE1), (0x10, 0x8E, 0xD2), (0x0F, 0xAE, 0xB9), (0x65, 0xBE, 0x86),
                    (0xC0, 0xBC, 0x60), (0xFF, 0xC3, 0x37), (0xF9, 0xFB, 0x0E))
        bitdepth = 8
        query_id = "demosubst"
        # cast args to int
        peak_id, job_id, sf_id, dataset_id = int(get_id_from_slug(peak_id)) if peak_id else None, int(job_id), int(
            sf_id), int(dataset_id)
        ...
        def res_dict():
            ...
            dimensions = self.db.query(
                "SELECT nrows,ncols FROM jobs j JOIN datasets d on j.dataset_id=d.dataset_id WHERE j.id=%d" % job_id)[
                0]
