...
def test_insert(sm_config, empty_test_db):
...
def test_insert_transaction_commit(sm_config, empty_test_db):
    with ConnectionPool(sm_config['db']):
        db1 = DB()
...
def test_insert_transaction_rollback(sm_config, empty_test_db):
    with ConnectionPool(sm_config['db']):
        db1 = DB()
        db1.alter(TABLE_CREATE)
