...
@patch(
    'sm.engine.mol_db.MolDBServiceWrapper.find_db_by_name_version',
    return_value=[mol_db_mock],
)
def test_valid_colocalization_jobs_generated(find_db_by_name_version_mock):
    ion_images = FreeableRef(
        np.array([np.linspace(0, 50, 50, False) % (i + 2) for i in range(20)])
    )
    ...
    sample_job = [
        job for job in jobs if job.fdr == 0.2 and job.algorithm_name == 'cosine'
    ][0]
...
def mock_get_ion_images_for_analysis(storage_type, img_ids, **kwargs):
    images = (
        np.array(
            [
                np.linspace(0, 25, 25, False) % ((seed or 1) % 25)
                for seed in range(len(img_ids))
            ],
            dtype=np.float32,
        )
        / 25
...
def test_new_ds_saves_to_db(find_db_by_name_version_mock, test_db, metadata, ds_config):
    ...
    job_id, = db.insert_return(
        "INSERT INTO job (db_id, ds_id, status) "
        "VALUES (1, %s, 'FINISHED') "
        "RETURNING id",
        [[ds.id]],
    )
    ...
    img_svc_mock.get_ion_images_for_analysis.side_effect = (
