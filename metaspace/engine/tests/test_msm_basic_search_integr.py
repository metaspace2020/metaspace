...
def make_fetch_formula_centroids_mock():
    def fetch_formula_centroids_mock(ion_formula_map_df):
        formulas_df = (
            pd.DataFrame({'formula_i': ion_formula_map_df.index, 'formula': ion_formula_map_df.ion_formula})
            .set_index('formula_i')
            .drop_duplicates()
        )
        centroids_df = (
...
def make_formula_image_metrics_mock_side_effect():
    def formula_image_metrics_mock(formula_images_it, compute_metrics, target_formula_inds, n_peaks):
        ...
        formula_metrics_df = pd.DataFrame([(i, 0.95) for i in formula_is], columns=['formula_i', 'msm']).set_index(
            'formula_i'
        )
...
def test_compute_fdr(spark_context, ds_config):
    ...
    formula_metrics_df = pd.DataFrame(
        [(10, 'H3O', 0.99), (11, 'C5H4O', 0.5), (12, 'H2ONa', 0.1)], columns=['formula_i', 'ion_formula', 'msm']
    ).set_index('formula_i')
...
def test_search(formula_image_metrics_mock, spark_context, ds_config):
    with TemporaryDirectory() as tmpdir:
        ...
        formulas_df = pd.DataFrame([(0, 'H3O'), (1, 'C5H4O')], columns=['formula_i', 'formula']).set_index('formula_i')
        centroids_df = (
            pd.DataFrame(
        ...
        image_metrics_result_list = [
            (pd.DataFrame([(0, 0.95)], columns=['formula_i', 'msm']).set_index('formula_i'), {0: np.zeros((1, 1))}),
            (
...
def test_ambiguous_modifiers(formula_image_metrics_mock, spark_context, ds_config):
    with TemporaryDirectory() as tmpdir:
        ...
        msm_search = MSMSearch(
            spark_context, make_imzml_parser_mock(), [make_moldb_mock(formulas)], ds_config, ds_data_path
        )
