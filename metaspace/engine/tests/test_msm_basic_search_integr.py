...
def make_fetch_formula_centroids_mock():
    def fetch_formula_centroids_mock(ion_formula_map_df):
        formulas_df = (
            pd.DataFrame(
                {
                    'formula_i': ion_formula_map_df.index,
...
def test_compute_fdr(spark_context, ds_config):
    ...
    formula_map_df = collect_ion_formulas(spark_context, moldb_fdr_list).drop(
        'moldb_id', axis=1
    )
...
def test_search(formula_image_metrics_mock, spark_context, ds_config):
    with TemporaryDirectory() as tmpdir:
        ...
        image_metrics_result_list = [
            (
                pd.DataFrame([(0, 0.95)], columns=['formula_i', 'msm']).set_index(
                    'formula_i'
                ),
                {0: np.zeros((1, 1))},
            ),
            (
                pd.DataFrame([(1, 0.9)], columns=['formula_i', 'msm']).set_index(
                    'formula_i'
                ),
...
def test_ambiguous_modifiers(formula_image_metrics_mock, spark_context, ds_config):
    with TemporaryDirectory() as tmpdir:
        ...
        formula_image_metrics_mock.side_effect = (
            make_formula_image_metrics_mock_side_effect()
        )
        ...
        assert (
            len(moldb_ion_metrics_df[moldb_ion_metrics_df.ion_formula == 'H7O2']) == 1
        )
