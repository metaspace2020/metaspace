...
def make_fetch_formula_centroids_mock():
    def fetch_formula_centroids_mock(ion_formula_map_df):
        formulas_df = (pd.DataFrame({'formula_i': ion_formula_map_df.index,
                                    'formula': ion_formula_map_df.ion_formula})
                       .set_index('formula_i')
                       .drop_duplicates())
        centroids_df = (pd.DataFrame(data=[(formula_i, peak_i, formula_i + peak_i, 100 - min(99, formula_i * peak_i))
...
def make_formula_image_metrics_mock_side_effect():
    def formula_image_metrics_mock(formula_images_it, compute_metrics, target_formula_inds, n_peaks):
        formula_is = set(item[0] for item in formula_images_it)
        formula_metrics_df = pd.DataFrame([(i, 0.95) for i in formula_is], columns=['formula_i', 'msm']).set_index('formula_i')
...
def test_search(formula_image_metrics_mock, spark_context, ds_config):
    with TemporaryDirectory() as tmpdir:
        ...
        print(ds_data_path)
        msm_search = MSMSearch(spark_context, make_imzml_parser_mock(), [make_moldb_mock()], ds_config, ds_data_path)
        ...
        msm_search.process_segments = lambda centr_segm_n, func: spark_context.parallelize(map(func, range(centr_segm_n)))
...
def test_ambiguous_modifiers(formula_image_metrics_mock, spark_context, ds_config):
    with TemporaryDirectory() as tmpdir:
        ...
        formulas = ['H3O', 'H4O', 'H5O2', 'H6O2'] # Formulae selected to create isobars with the above modifiers
        msm_search = MSMSearch(spark_context, make_imzml_parser_mock(), [make_moldb_mock(formulas)],
                               ds_config, ds_data_path)
        ...
        assert moldb_ion_metrics_df[['formula','chem_mod','neutral_loss','adduct']].duplicated().sum() == 0
