...
def test_if_not_exist_returns_valid_df(pyspark_context, ds_config, clean_isotope_storage_path):
    ...
    centroids_gen = CentroidsGenerator(sc=pyspark_context, moldb_name='HMDB', isocalc=isocalc)
    ...
    assert ion_centroids.formulas_df.shape == (2, 2)


def test_save_restore_works(pyspark_context, ds_config, clean_isotope_storage_path):
    ion_centroids = FormulaCentroids(formulas_df=pd.DataFrame({'ion_i': [101, 101, 102, 102],
    ...
    isocalc = IsocalcWrapper(ds_config['isotope_generation'])
    centr_gen = CentroidsGenerator(sc=pyspark_context, moldb_name='HMDB', isocalc=isocalc)
    centr_gen._save(ion_centroids)
    ion_centroids_restored = centr_gen._restore()

    from pandas.testing import assert_frame_equal
    assert_frame_equal(ion_centroids.formulas_df.sort_index(), ion_centroids_restored.ions_df.sort_index())
...
def test__generate_if_not_exist__new_custom_adduct(pyspark_context, ds_config, clean_isotope_storage_path):
    isocalc = IsocalcWrapper(ds_config['isotope_generation'])
    centr_gen = CentroidsGenerator(sc=pyspark_context, moldb_name='HMDB', isocalc=isocalc)
    ion_centroids = FormulaCentroids(formulas_df=pd.DataFrame({'ion_i': [0, 1],
                                                       'formula': ['H2O', 'Au'],
                                                       'adduct': ['+H', '+H']})
                                     .set_index('ion_i'),
