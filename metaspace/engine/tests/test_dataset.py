...
def test_dataset_load_existing_ds_works(fill_db, sm_config, ds_config):
    ...
    assert (ds.id == ds_id and ds.name == 'ds_name' and ds.input_path == 'input_path' and
            ds.upload_dt == upload_dt and ds.metadata == metadata and ds.config == ds_config and
...
def test_dataset_save_overwrite_ds_works(fill_db, sm_config, ds_config):
    ...
    ds_id = '2000-01-01'
    ds = Dataset(ds_id, 'ds_name', 'input_path', upload_dt, {}, ds_config, mol_dbs=['HMDB'])
...
def test_dataset_update_status_works(fill_db, sm_config, ds_config):
    ...
    upload_dt = datetime.now()
    ds_id = '2000-01-01'
    ds = Dataset(ds_id, 'ds_name', 'input_path', upload_dt, {}, ds_config, DatasetStatus.INDEXING, mol_dbs=['HMDB'])
...
def test_dataset_to_queue_message_works():
    ...
    ds = Dataset(ds_id, 'ds_name', 'input_path', upload_dt, meta, ds_config, mol_dbs=['HDMB'])
