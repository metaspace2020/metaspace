...
wait
ssh ubuntu@sm-webserver "luigid --background --logdir /home/ubuntu/luigi_logs"

wait

echo "  === [2] Code deployment to SM webserver ===  "
ssh ubuntu@sm-webserver "mkdir -p /home/ubuntu/sm/data"
rsync  --exclude=.* -rv ../../webserver ubuntu@sm-webserver:/home/ubuntu/sm
...
ssh ubuntu@sm-webserver "/opt/dev/spark-1.4.1-bin-hadoop2.4/ec2/spark-ec2 --key-pair=sm_spark_cluster --identity-file=/home/ubuntu/.ssh/sm_spark_cluster.pem --region=eu-west-1 --slaves=${SLAVES_N} --instance-type=m3.large --master-instance-type m3.medium --copy-aws-credentials --ami=ami-c49acbb3 --spot-price=0.03 --private-ips launch ${CLUSTER_NAME}"
