...
def calc_hash(object, chunk_size=1024 * 1024):
...
def main():
    ...
    for it in df.sort_values(by='Size').to_dict('records'):
        if it['Key'].startswith('databases'):
            continue
        filename = it['Key'].split('/')[-1]
        if filename.lower().endswith('ibd') or filename.lower().endswith('imzml'):
            try:
                resp = conn.get_object(Bucket=args.bucket, Key=it['Key'], VersionId=it['VersionId'])
                start = time.time()
                item = {}
                ...
                data.append(item)
                logging.info(
                    f'{item["md5"]} {item["sha256"]} {item["calc_time"]:>9.3f} {item["size"]} {item["version_id"]} {item["key"]}'
                )
            except botocore.exceptions.ClientError:
                logging.error(f'{it["Key"]}')
