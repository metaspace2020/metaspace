...
def test_sf_image_metrics(spark_context, ds_formulas_images_mock, ds_config):
    with patch('engine.formula_img_validator.get_compute_img_measures') as mock:
        ...
        exp_metrics_df = pd.DataFrame([[0, 0.9, 0.9, 0.9, 0.9**3],
                                       [1, 0.9, 0.9, 0.9, 0.9**3]],
                                      columns=['index', 'chaos', 'spatial', 'spectral', 'msm']).set_index('index')
...
def test_add_sf_image_est_fdr():
    ...
    formulas_mock = MagicMock(spec=Formulas)
    formulas_mock.get_sf_adduct_peaksn.return_value = [[0, '+H', 0], [1, '+H', 0]]

    fdr_mock = MagicMock(spec=FDR)
    fdr_mock.estimate_fdr.return_value = pd.DataFrame([[0, 0, '+H', 0.99], [1, 1, '+H', 0.5]],
                                                      columns=['index', 'sf_id', 'adduct', 'fdr']).set_index('index')

    res_metrics_df = sf_image_metrics_est_fdr(sf_metrics_df, formulas_mock, fdr_mock)

    exp_col_list = ['index', 'sf_id', 'adduct', 'chaos', 'spatial', 'spectral', 'msm', 'fdr']
    exp_metrics_df = pd.DataFrame([[0, 0, '+H', 0.9, 0.9, 0.9, 0.9**3, 0.99],
                                   [1, 1, '+H', 0.5, 0.5, 0.5, 0.5**3, 0.5]],
                                  columns=exp_col_list).set_index('index')
