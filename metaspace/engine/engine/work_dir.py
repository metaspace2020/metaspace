...
from engine.util import local_path, proj_root, cmd_check, cmd, SMConfig, logger, s3_path
...
class LocalWorkDir(object):
    ...
    @property
    def imzml_path(self):
        return glob.glob(join(self.ds_path, '*.imzML'))[0]
...
class WorkDirManager(object):
    ...
    def copy_input_data(self, input_data_path, ds_config_path):
        ...
        if not self.exists(self.txt_path):
