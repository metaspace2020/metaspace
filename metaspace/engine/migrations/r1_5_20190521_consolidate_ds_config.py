...
def migrate(db, output_file):
    ...
    # Backup
    to_backup = [[id, config, mol_dbs, adducts]
    ...
    db.alter_many("UPDATE dataset "
                  ...
                  "WHERE dataset.id = u.id", new_configs)
...
def restore(db, backup_file):
    ...
    backup = [(id, json.dumps(config), mol_dbs, adducts)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--revert', action='store_true', help="Revert by extracting data from config (potentially lossy)")
    parser.add_argument('--restore-backup', action='store_true', help="Restore from file (lossless)")
