...
from sm.engine.serverless.fdr import build_fdr_rankings, calculate_fdrs, calculate_fdrs_vm
...
class Pipeline(object):
    def __init__(
        self, config, ds_config, db_config, use_db_cache=True, use_ds_cache=True, vm_algorithm=True
    ):
        self.config = config
        ...
        self.pywren_executor = lithops.function_executor(config=self.config, runtime_memory=2048)
        self.pywren_vm_executor = lithops.docker_executor(
    ...
    def build_database(self, use_cache=True, debug_validate=False):
        if self.vm_algorithm:
            ...
            if use_cache and self.cacher.exists(cache_key):
                ...
                self.pywren_vm_executor.call_async(
                ...
                (
                    ...
                    build_db_exec_time,
                ) = self.pywren_vm_executor.get_result()
    ...
    def split_ds(self, use_cache=True):
        ...
        if self.vm_algorithm:
            ...
            if use_cache and self.cacher.exists(cache_key):
                ...
                self.ds_chunks_cobjects = chunk_spectra(
    ...
    def segment_ds(self, use_cache=True, debug_validate=False):
        ...
        if self.vm_algorithm:
            ...
            if use_cache and self.cacher.exists(cache_key):
                ...
                self.cacher.save(
                    (self.ds_segments_bounds, self.ds_segms_cobjects, self.ds_segms_len), cache_key
                )
    ...
    def segment_centroids(self, use_cache=True, debug_validate=False):
        ...
        if use_cache and self.cacher.exists(cache_key):
            ...
            self.clip_centr_chunks_cobjects, centr_n = clip_centr_df(
                self.pywren_executor, self.peaks_cobjects, mz_min, mz_max
            )
    ...
    def get_images(self):
        ...
        futures = self.pywren_executor.map(
            get_target_images, self.images_cloud_objs, runtime_memory=1024
        )
        all_images = {}
