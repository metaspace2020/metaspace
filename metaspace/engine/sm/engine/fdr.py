...
class FDR(object):
    ...
    def decoy_adducts_selection(self, target_formulas):
        decoy_adduct_cand = [
            add for add in DECOY_ADDUCTS if add not in self.target_adducts
        ]
    ...
    def estimate_fdr(self, formula_msm):
        ...
        for tm in self.target_modifiers_df.index.drop_duplicates():
            ...
            msm_fdr_avg = pd.Series(
                pd.concat(msm_fdr_list, axis=1).median(axis=1), name='fdr'
            )
