...
def isotope_image_correlation_sd(images_as_2darray):
    ...
    images_as_2darray = np.nan_to_num(images_as_2darray)
...
def stats_signaltonoise(a, axis=0, ddof=0):
    """
    scipy.stats.signaltonoise was deprecated in 0.16.0 and removed in 1.0. Here's the source code
    from approximately the time that the old metrics were written:
    https://github.com/scipy/scipy/blob/maintenance/0.17.x/scipy/stats/stats.py#L2302
    """
    a = np.asanyarray(a)
    m = a.mean(axis)
...
def snr_img(image: np.ndarray):
    ...
    :rtype float
    """
    snr = float(stats_signaltonoise(image, axis=None))
    return snr


def percent_zero(image: np.ndarray):
    """
...
def spectra_int_diff(images_flat: np.ndarray, theor_iso_intensities: np.ndarray):
    """
    ...
    while len(int_diff) < 4:
        int_diff = np.append(
            int_diff, 2.01
        )  # in the case where one of the peaks is missing the difference is maximum

    return int_diff[:4]


def quartile_pxl(image: np.ndarray):
    """
    ...
    """
    image = image / np.linalg.norm(image)  # EDITED FROM ORIGINAL TO NOT MODIFY THE ORIGINAL IMAGE
    quart = stats.mstats.mquantiles(image)

    return quart


def decile_pxl(image: np.ndarray):
    """
    ...
    """
    image = image / np.linalg.norm(image)  # EDITED FROM ORIGINAL TO NOT MODIFY THE ORIGINAL IMAGE
    decile = stats.mstats.mquantiles(
        image, prob=[0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]
    )

    return decile


def ratio_peaks(images_flat: np.ndarray):
    """
