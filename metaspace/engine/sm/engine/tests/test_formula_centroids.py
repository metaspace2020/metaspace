...
def create_formula_centroids(rows1, rows2):
    return FormulaCentroids(
        ...
        centroids_df=(pd.DataFrame(rows2, columns=['formula_i', 'peak_i', 'mz', 'int']).set_index('formula_i')),
    )
...
def test_add_overlap_exception():
    formula_centroids = create_formula_centroids(
        [(0, 'f1a1'), (1, 'f2a2')], [(0, 0, 10, 100), (0, 1, 15, 100), (1, 0, 20, 100), (1, 1, 25, 100)]
    )
    formula_centroids_other = create_formula_centroids([(0, 'f2a2')], [(0, 0, 20, 100), (0, 1, 25, 100)])
