...
def create_formula_centroids(rows1, rows2):
    return FormulaCentroids(
        formulas_df=(
            pd.DataFrame(rows1, columns=['formula_i', 'formula']).set_index('formula_i')
        ),
        centroids_df=(
            pd.DataFrame(rows2, columns=['formula_i', 'peak_i', 'mz', 'int']).set_index(
                'formula_i'
            )
        ),
    )
...
def test_add_non_overlap():
    f_centr = create_formula_centroids(
        [(0, 'f1a1')], [(0, 0, 10, 100), (0, 1, 20, 100)]
    )
    ...
    assert (
        f_centr.formulas_df.index.name
        == f_centr.centroids_df().index.name
        == 'formula_i'
    )
