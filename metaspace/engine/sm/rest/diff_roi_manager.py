...
class DiffROIData:
    ...
    def get_dataset_roi(self):
        with ConnectionPool(self._sm_config['db']):
            result = self._db.select_one('SELECT roi FROM dataset WHERE id = %s', params=(self.ds_id,))
    ...
    def get_ppm(self):
        with ConnectionPool(self._sm_config['db']):
            ppm = self._db.select_one("SELECT config->'image_generation'->>'ppm' FROM dataset WHERE id = %s",params=(self.ds_id,))
        ppm = int(ppm[0])
        return ppm
    ...
    def get_annots_with_metrics(self):
        ...
        for db_res in result:
            ...
            decoy_map = self._image_storage.get_image(self._image_storage.DIAG, self.ds_id, decoy_map_img_id)
            sf_map = self._image_storage.get_image(self._image_storage.DIAG, self.ds_id, formula_map_img_id)
            metrics_df_bytes = self._image_storage.get_image(self._image_storage.DIAG, self.ds_id, metrics_df_img_id)
    ...
    def get_TIC_image(self):
        ...
        img_bytes = self._image_storage.get_image(self._image_storage.DIAG, self.ds_id, TIC_image_id)
...
class DiffROIAnalysis:
    def __init__(self, ds_id, hotspot_percentile: int = 99,
        ...
        self.ds_id = ds_id
        self.data =  DiffROIData(self.ds_id, hotspot_percentile,
                                      TIC_normalize,
                                      log_transform_tic)
    ...
    def run_diff_roi(self):
        ...
        def build_ion_images_chunk(lefts, rights, ints, sp_idxs, n_pixels,
            ...
            for i in range(chunk_size):
                low, high = lefts[chunk_start + i], rights[chunk_start + i]
                if low < high:
                    chunk[i] = np.bincount(
