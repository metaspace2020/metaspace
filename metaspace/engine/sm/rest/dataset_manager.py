...
class SMapiDatasetManager(object):
    ...
    def _set_ds_busy(self, ds, ignore_status=False):
        if (
            ds.status in {DatasetStatus.QUEUED, DatasetStatus.ANNOTATING}
    ...
    def add(self, doc, **kwargs):
        ...
        self._post_sm_msg(
            ds=ds, queue=self._annot_queue, action=DaemonAction.ANNOTATE, **kwargs
        )
        return doc['id']
    ...
    def delete(self, ds_id, **kwargs):
        ...
        self._set_ds_busy(ds, kwargs.get('force', False))
        self._post_sm_msg(
            ds=ds, queue=self._update_queue, action=DaemonAction.DELETE, **kwargs
        )
    ...
    def add_optical_image(self, ds_id, img_id, transform, zoom_levels=(1, 2, 4, 8)):
        ...
        add_optical_image(
            self._db, self._img_store, ds_id, img_id, transform, zoom_levels
        )
