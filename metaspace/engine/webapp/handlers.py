...
class IsoImgBaseHandler(tornado.web.RequestHandler):
    ints_sql = '''SELECT intensities as ints
                    FROM job_result_data d
                    JOIN job j ON d.job_id=j.id
                    WHERE j.id=%s AND d.sf_id=%s
                    ORDER BY peak'''
    dim_sql = '''SELECT nrows, ncols
                FROM job j
                JOIN dataset d on d.id=j.ds_id
                WHERE j.id=%d'''
    coord_sql = '''SELECT x,y FROM coordinates WHERE ds_id=%s ORDER BY index'''
    # Viridis
    ...
    def res_dict(self, ds_id, job_id, sf_id):
        coords_rows = self.db.query(self.coord_sql % int(ds_id))
        coords = [r.values() for r in coords_rows]

        dimensions_rows = self.db.query(self.dim_sql % (job_id))[0]
        rows, cols = dimensions_rows.values()
        res_list_rows = self.db.query(self.ints_sql % (job_id, sf_id))
        intens_list = np.array([np.array(r.values()[0]) for r in res_list_rows])
...
class IsoImgPngHandler(IsoImgBaseHandler):
    theor_peaks_sql = "select centr_mzs[%s] from theor_peaks where db_id = %s and sf_id = %s and adduct = %s;"

    @gen.coroutine
    def get(self, *args):
        db_id, ds_id, job_id, sf_id, sf, adduct, peak_id = args
        ints_list, coords, rows, cols = self.res_dict(int(ds_id), int(job_id), int(sf_id))

        peak_mz = self.db.query(self.theor_peaks_sql, int(peak_id)+1, int(db_id), int(sf_id), adduct)[0].values()[0]
