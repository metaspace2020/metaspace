...
class IsoImgBaseHandler(tornado.web.RequestHandler):
    ...
    def _get_intens_list(self, job_id, sf_id):
    ...
    def get_img_ints(self, *args):
        pass

    def _get_color_image_data(self, ds_id, job_id, sf_id):
        coords_rows = self.db.query(self.coord_sql % int(ds_id))
        coords = np.array([r.values() for r in coords_rows])
        coords -= coords.min(axis=0)
        nrows, ncols = coords.max(axis=0) + 1

        visible_pixels = self.get_img_ints(self._get_intens_list(job_id, sf_id)).reshape((nrows, ncols))
        ...
        mask = self._get_ds_mask(coords, nrows, ncols)
...
class IsoImgPngHandler(IsoImgBaseHandler):
    ...
    @gen.coroutine
    def get(self, *args):
        db_id, ds_id, job_id, sf_id, sf, adduct, self.peak_id = args
        ds_id = int(ds_id)
        job_id = int(job_id)
        sf_id = int(sf_id)
